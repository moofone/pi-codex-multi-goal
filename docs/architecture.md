...
- **Human inspection (invariants 5, 6):** `/goal-monitor` launches a localhost
  dashboard on a random free port (command only; never auto-start). It shows
  current working memory, the full step/criteria sequence and its history, each
  DAG admission (the exact snapshot and surviving messages brought into a new
  step after isolation), and how many isolated steps versus host
  `session_compact` events would have been compacted. The page is not model
  context.
