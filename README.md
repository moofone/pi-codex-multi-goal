...
/goal-monitor       launch a localhost dashboard on a random free port (live memory, steps, DAG isolation)
```
...
`/goal-monitor` binds `127.0.0.1` on a random free port and does not inject
anything into model context. Re-running the command reuses the same server. The
dashboard follows working memory, the full step/criteria sequence, each DAG
admission (the exact snapshot and surviving messages brought into a new step),
and how many isolated steps versus host `session_compact` events would have been
compacted.
